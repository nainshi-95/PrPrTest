...
PYBIND11_MODULE(tf_motion, m) {
    m.doc() = "Standalone pybind11 wrapper for VTM-style temporal filter motion estimation/apply-motion (luma only, external padding required)";
...
Important:
    Input must already be externally padded in Python.
    Input height/width must be multiples of 16.

Args:
    target:    2D numpy array, shape (H, W)
    reference: 2D numpy array, same shape/dtype as target
    bitdepth:  Optional explicit bit depth. Default: infer from dtype
...
Important:
    Reference must be the same externally padded image domain used for motion estimation.
...
def pad_for_tf_motion(img: np.ndarray, pad=64, multiple=16):
    assert img.ndim == 2

    h, w = img.shape

    # 1) 상하좌우 고정 패딩
    out = np.pad(img, ((pad, pad), (pad, pad)), mode="edge")

    # 2) 최종 크기를 16의 배수로 맞춤
    h2, w2 = out.shape
    pad_bottom = (multiple - (h2 % multiple)) % multiple
    pad_right  = (multiple - (w2 % multiple)) % multiple

    out = np.pad(out, ((0, pad_bottom), (0, pad_right)), mode="edge")
    ...
    meta = {
        ...
        "pad_top": pad,
        "pad_left": pad,
        "pad_bottom_extra": pad_bottom,
        "pad_right_extra": pad_right,
        "final_h": out.shape[0],
        "final_w": out.shape[1],
    }
    return out, meta
...
def crop_from_tf_motion(img_pad: np.ndarray, meta):
    y0 = meta["pad_top"]
    x0 = meta["pad_left"]
    h = meta["orig_h"]
    w = meta["orig_w"]
    return img_pad[y0:y0+h, x0:x0+w]
...
target_pad, meta = pad_for_tf_motion(target, pad=64, multiple=16)
reference_pad, _ = pad_for_tf_motion(reference, pad=64, multiple=16)

mv = tf_motion.estimate_motion(target_pad, reference_pad, bitdepth=10)
pred_pad = tf_motion.apply_motion(mv, reference_pad, bitdepth=10)
pred = crop_from_tf_motion(pred_pad, meta)
