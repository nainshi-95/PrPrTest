    ...
    for (int blockY = 0; blockY + blockSize <= origHeight; blockY += stepSize) {
        for (int blockX = 0; blockX + blockSize <= origWidth; blockX += stepSize) {
            ...
            if (previous == nullptr) {
                ...
                const int64_t zero_error = motion_error_luma(orig, buffer, blockX, blockY, 0, 0,
                                                             blockSize, best.error, maxValue);
...
Array2D<MotionVector> motion_estimation(const Image& orgPic, const Image& buffer, int bitdepth) {
    ...
    if (orgPic.w % 16 != 0 || orgPic.h % 16 != 0) {
        throw std::runtime_error("input height and width must be multiples of 16");
    }

    const Image origSub2 = subsample_luma(orgPic, 2);
    const Image origSub4 = subsample_luma(origSub2, 2);
    const Image bufSub2  = subsample_luma(buffer, 2);
    const Image bufSub4  = subsample_luma(bufSub2, 2);

    Array2D<MotionVector> mv0(origSub4.w / 16, origSub4.h / 16);
    Array2D<MotionVector> mv1(origSub2.w / 16, origSub2.h / 16);
    ...
    motion_estimation_luma(mv2, orgPic,   buffer,  16, bitdepth, &mv1, 2);
    motion_estimation_luma(mv,  orgPic,   buffer,   8, bitdepth, &mv2, 1, true);
        ...
        for (int bx = 0; bx < mvs.w(); ++bx) {
            ...
            for (int iy = 0; iy < 8; ++iy) {
                for (int ix = 0; ix < 8; ++ix) {
                    out.at(bx * 8 + ix, by * 8 + iy) =
                        filtered_sample_6tap(ref, x0 + ix, y0 + iy, mv.x, mv.y, maxValue);
                }
            }
        }
    }
...
PYBIND11_MODULE(tf_motion, m) {
    m.doc() = "Standalone pybind11 wrapper for VTM-style temporal filter motion estimation/apply-motion (luma only, external padding optional but recommended)";
...
Input:
    motion:    int16 numpy array, shape (H/8, W/8, 2)
...
Array2D<MotionVector> motion_estimation(const Image& orgPic, const Image& buffer,
                                        int bitdepth, int finalBlockSize) {
    validate_final_block_size(finalBlockSize);

    const int coarseBlockSize = finalBlockSize * 2;
    const int requiredMultiple = finalBlockSize * 8;  // sub4 + coarse(2B) alignment

    if (orgPic.w != buffer.w || orgPic.h != buffer.h) {
        throw std::runtime_error("target and reference must have the same shape");
    }
    if (orgPic.w % requiredMultiple != 0 || orgPic.h % requiredMultiple != 0) {
        throw std::runtime_error("input height and width must be multiples of 8 * final_block_size");
    }
    ...
    Array2D<MotionVector> mv0(origSub4.w / coarseBlockSize, origSub4.h / coarseBlockSize);
    Array2D<MotionVector> mv1(origSub2.w / coarseBlockSize, origSub2.h / coarseBlockSize);
    Array2D<MotionVector> mv2(orgPic.w / coarseBlockSize,   orgPic.h / coarseBlockSize);
    Array2D<MotionVector> mv (orgPic.w / finalBlockSize,    orgPic.h / finalBlockSize);

    motion_estimation_luma(mv0, origSub4, bufSub4, coarseBlockSize, bitdepth);
    motion_estimation_luma(mv1, origSub2, bufSub2, coarseBlockSize, bitdepth, &mv0, 2);
    motion_estimation_luma(mv2, orgPic,   buffer,  coarseBlockSize, bitdepth, &mv1, 2);
    motion_estimation_luma(mv,  orgPic,   buffer,  finalBlockSize,  bitdepth, &mv2, 1, true);

    return mv;
}

Image apply_motion(const Array2D<MotionVector>& mvs, const Image& ref,
                   int bitdepth, int finalBlockSize) {
    validate_final_block_size(finalBlockSize);
    ...
    if (mvs.w() != ref.w / finalBlockSize || mvs.h() != ref.h / finalBlockSize) {
        throw std::runtime_error("motion shape must be (H/final_block_size, W/final_block_size, 2)");
    }
    ...
    for (int by = 0; by < mvs.h(); ++by) {
        for (int bx = 0; bx < mvs.w(); ++bx) {
            ...
            for (int iy = 0; iy < finalBlockSize; ++iy) {
                for (int ix = 0; ix < finalBlockSize; ++ix) {
                    out.at(x0 + ix, y0 + iy) =
                        filtered_sample_6tap(ref, x0 + ix, y0 + iy, mv.x, mv.y, maxValue);
                }
            }
...
py::array estimate_motion_py(const py::array& target, const py::array& reference,
    ...
    auto [rimg, bd1] = parse_image_and_bitdepth(reference, bitdepth);
    const int bd = std::max(bd0, bd1);
    const auto mvs = motion_estimation(timg, rimg, bd, final_block_size);
    return motion_to_numpy(mvs);
}
...
py::array apply_motion_py(const py::array_t<int16_t, py::array::c_style | py::array::forcecast>& motion,
    ...
    auto [rimg, bd] = parse_image_and_bitdepth(reference, bitdepth);
    const auto mvs = numpy_to_motion(motion);
    const Image out = apply_motion(mvs, rimg, bd, final_block_size);
...
PYBIND11_MODULE(tf_motion, m) {
    m.doc() = "Standalone pybind11 wrapper for VTM-style temporal filter motion estimation/apply-motion (luma only, configurable final_block_size)";
...
Input:
    2D numpy arrays with same shape.
    Height/width must be multiples of (8 * final_block_size).
...
Returns:
    int16 numpy array of shape (H/final_block_size, W/final_block_size, 2)
    Motion vectors are in 1/16-pel units
)pbdoc");
...
Input:
    motion: int16 numpy array, shape (H/final_block_size, W/final_block_size, 2)
    reference: 2D numpy array, shape (H, W)
    bitdepth: optional explicit bitdepth
    final_block_size: 8 or 16
